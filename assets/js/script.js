...
// Auto-scroll active filter button closer to left side on mobile
document.addEventListener('DOMContentLoaded', function () {
	// Only run on mobile/tablet sizes
	if (window.innerWidth <= 767) {
		const filterButtons = document.querySelectorAll('.filter-tabs .filter');

		filterButtons.forEach(button => {
			button.addEventListener('click', function () {
				// Give isotope a tiny moment to update (if needed)
				setTimeout(() => {
					const parent = this.parentElement; // .filter-tabs
					const rect = this.getBoundingClientRect();
					const parentRect = parent.getBoundingClientRect();

					// Calculate how far to scroll so selected button is ~20-40px from left edge
					const scrollOffset = rect.left - parentRect.left - 24; // 24px = nice left padding

					parent.scrollTo({
						left: parent.scrollLeft + scrollOffset,
						behavior: 'smooth'
					});
				}, 50); // small delay helps if isotope is still animating
			});
		});

		// Also scroll to active button on page load (nice touch)
		const activeBtn = document.querySelector('.filter-tabs .filter.active');
		if (activeBtn) {
			setTimeout(() => {
				const parent = activeBtn.parentElement;
				const rect = activeBtn.getBoundingClientRect();
				const parentRect = parent.getBoundingClientRect();
				const scrollOffset = rect.left - parentRect.left - 24;
